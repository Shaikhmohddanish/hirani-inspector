...
import { generateModifiedReport } from "@/lib/reports";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const reportBuffer = await generateModifiedReport(imageIds);

    return new NextResponse(Uint8Array.from(reportBuffer), {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "Content-Disposition": `attachment; filename="inspection-report-annotated-${new Date().toISOString().split("T")[0]}.docx"`,
